...
public class PoiOverlay extends OverlayManager {
    ...
    private List<PoiResult> mPoiResultList = null;
    ...
    public void setDataList(List<PoiResult> list) {
        mPoiResultList = list;
    }

    @Override
    public final List<OverlayOptions> getOverlayOptions() {
        if (mPoiResultList == null) {
            return null;
        }
        for (int i = 0; i < mOverlayList.size(); i++) {
            if (mPoiResultList.get(i).getAllPoi() == null) {
                return null;
            }
        }
        ...
        for (int j = 0; j < mPoiResultList.size(); j++) {
            for (int i = 0; i < mPoiResultList.get(j).getAllPoi().size()
                    && markerSize < MAX_POI_SIZE; i++) {
                mPoiResult = mPoiResultList.get(j);
